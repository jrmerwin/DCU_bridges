...
Here we investigate two complementary routes. A **horizontal transfer** takes a fixed registry-based parameter dictionary, an explicitly specified physical law, and any declared experimental inputs, and computes an observable not identical to the parameter itself. A **vertical bridge** starts from native structure or native activity and maps it to an operational measurement through a stated attachment. The first route tests the utility and limits of the parameter prescriptions. The second tests what actual DCU processes can support and what a specified observer could infer.
...
## Definitions, source choices and evidence conventions

### Native construction and service
...
### Record values and the quantum attachment
...
### Fixed dictionary and external inputs
...
### Mass prescriptions into weak partial rates
...
### Recoil and reduced-mass spectroscopy
...
### Diagnosing a missing physical term with a measured-input control
...
### Neutrino waveforms, detector folding and released spectral information
...
### Thresholds, mixed sectors and the present boundary
...
## Vertical bridges from native activity

### Recording back-action and shared-record clock geometry
...
### A full native-history quantum channel
...
## New information and inverse-measurement bridges

### Shared native records as Shannon information
...
### The registry as a conditional-information graph
...
The positive-information graph has 5,347 edges and recovers the exact junction/two-branch form of D1. Each full branch bundle contains 17 distinct ancestral registers; their only shared register is c, so the two whole bundles are conditionally independent given g_c under this first-write model \cite{R38}.
...
### Inferring native activity without exposing hidden counts
...
| State | True total pool R | True p | Median estimated p, Q=27 | Relative RMSE in p |
|---|---:|---:|---:|---:|
...
### Recording workload and information are not interchangeable
...
## Implementation and identification boundaries
...
## Categorical reach and conclusion

| Domain | Demonstrated reach | Present boundary |
|---|---|---|
